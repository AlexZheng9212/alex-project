 ...
 */
public class ResizableBlockingQueue<E> extends AbstractQueue<E> implements BlockingQueue<E>, java.io.Serializable {

  /*
   * A variant of the "two lock queue" algorithm. The putLock gates entry to put
   * (and offer), and has an associated condition for waiting puts. Similarly for
   * the takeLock. The "count" field that they both rely on is maintained as an
   * atomic to avoid needing to get both locks in most cases. Also, to minimize
   * need for puts to get takeLock and vice-versa, cascading notifies are used.
   * When a put notices that it has enabled at least one take, it signals taker.
   * That taker in turn signals others if more items have been entered since the
   * signal. And symmetrically for takes signalling puts. Operations such as
   ...
   * subsequent reader guarantees visibility to the enqueued Node by either
   * acquiring the putLock (via fullyLock) or by acquiring the takeLock, and then
   * reading n = count.get(); this gives visibility to the first n items.
   *
   ...
   *
   */
  private static final long serialVersionUID = -6755665112103486829L;
   ...
   * traversals must handle both: - dequeued nodes (p.next == p) - (possibly
   ...
   *
   * Lazily updated ancestor field provides expected O(1) remove(), but still O(n)
   * in the worst case, whenever the saved ancestor is concurrently deleted.
   */
  ...
  /**
   * A customized variant of Spliterators.IteratorSpliterator. Keep this class in
   * sync with (very similar) LBDSpliterator.
   */
  ...
  /**
   * Runs action on each element found during a traversal starting at p. If p is
   * null, traversal starts at head.
   */
